...
interface EditProps {
  ...
  handleSelectedVideo: (video: Video | null) => void;
}

const Edit: React.FC<EditProps> = ({ selectedVideo, handleSelectedVideo }) => {
  const { handleUpdatedList, fetchedUserVideos } = useContext<any>(VideoContext);
  ...
  const handleDropdownSelection = (option: string, item: string) => {
  ...
  const getVideo = async (video: Video | null) => {
    if (!video || !userId) {
