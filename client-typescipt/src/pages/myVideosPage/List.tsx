...
interface ListProps {
  handleSelectedVideo: (video: any) => void;
}

const List: React.FC<ListProps> = ({ handleSelectedVideo }) => {
  const { updatedList, handleFetchedUserVideos, fetchedUserVideos } = useContext<any>(VideoContext);
  ...
  useEffect(() => {
    ...
    const fetchUserVideos = async (userId: string) => {
      const myVideos = await axios.get(`${process.env.REACT_APP_NODE_API}api/v1/auth/userVideos/${userId}`);
      ...
      {!isLoading ? (
        !showRecent ? null : (
          <ul className="sm:grid sm:grid-cols-[repeat(auto-fit,minmax(200px,1fr))] sm:gap-4">
            {fetchedUserVideos.map((card: any) => (
              <li
                key={card._id}
