...
const Navbar: React.FC = () => {
  ...
  const { searchVideo, handleShowUpload } = useContext<any>(VideoContext);
  ...
  const search = async (searchText: string) => {
    try {
      const result:any = await searchByTitle(searchText);
          ...
          {!auth?.user ? (
            <div className="invisible md:visible">
              <div className="flex gap-3 mt-3">
                <NavLink to="/login" className="block px-3 py-2 rounded-md text-base font-medium text-gray-300 hover:text-white">
                  Login
                </NavLink>
                <p className="block px-3 py-2 rounded-md text-base font-medium text-gray-300">|</p>
                <NavLink to="/register" className="block px-3 py-2 rounded-md text-base font-medium text-gray-300 hover:text-white">
                  Register
                </NavLink>
              </div>
            </div>
            ...
            <div className="invisible sm:visible">
              <div className="flex gap-3 mt-3">
                <NavLink to="/myvideos" className="block px-3 py-2 rounded-md text-base font-medium text-gray-300 hover:text-white">
                  My Videos
                </NavLink>
                <p className="block px-3 py-2 rounded-md text-base font-medium text-gray-300">|</p>
                <NavLink
                ...
                <p className="block px-3 py-2 rounded-md text-base font-medium text-gray-300">|</p>
                <NavLink to="/" onClick={handleLogout} className="block px-3 py-2 rounded-md text-base font-medium text-gray-300 hover:text-white">
                  Sign out
                </NavLink>
              </div>
            </div>
      ...
      {toggle && !auth?.user && (
        <div className="md:hidden" id="mobile-menu">
          <div className="px-2 pt-2 pb-3 space-y-1">
            <NavLink to="/login" className="block px-3 py-2 rounded-md text-base font-medium text-white hover:text-white">
              Login
            </NavLink>
            <NavLink to="/register" className="block px-3 py-2 rounded-md text-base font-medium text-white hover:text-white ">
              Register
            </NavLink>
          </div>
        </div>
